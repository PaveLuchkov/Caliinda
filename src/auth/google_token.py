...
# import src.shared.database as db_utils
from src.shared.types import TokenExchangeRequest
    ...
    try:
        ...
        if not creds.valid: # Проверка, есть ли валидный access_token
            if creds.expired and creds.refresh_token: # Если истек и есть refresh_token
                ...
                # Этого не должно случиться, если мы передали refresh_token
                # и он должен быть использован для получения access_token
                # Но если creds.valid false, а refresh() не был вызван (например, нет refresh_token в creds),
                # то access_token не будет.
                # В нашем случае, мы всегда хотим обновить/получить.
                # Можно просто всегда вызывать refresh, если есть refresh_token
